...
`Chrome 확인`, 쿠팡 검색 `Chrome에서 더 보기`, RedNote 검색은 사용자가 로그인해 둔 Google Chrome 프로필을 사용합니다. Chrome 메뉴의 `보기 → 개발자 → Apple Events의 자바스크립트 허용`을 켜고, macOS의 `시스템 설정 → 개인정보 보호 및 보안 → 자동화`에서 실행에 사용하는 터미널 앱(필요한 경우 Python 또는 Node)이 Google Chrome을 제어하도록 허용하세요. Apple Events 권한을 거부했다면 이 설정에서 다시 켠 뒤 스튜디오를 재시작합니다.

## 6단계 통합 흐름

1. **계정** — `현재 계정 가져오기`로 OAuth를 완료하고, 연결된 Threads 계정 중 실제 발행 대상을 선택합니다.
2. **상품** — 상품명으로 쿠팡 파트너스 결과를 먼저 최대 10개 확인하고, `Chrome에서 더 보기`로 로그인된 Chrome 검색 결과를 합쳐 최대 30개까지 비교합니다. 상품을 선택하면 해당 상품의 파트너스 딥링크를 만들며, 필요하면 URL을 직접 입력할 수 있습니다.
3. **RedNote** — 쿠팡 상세 분석에서 확인된 상품 타이틀을 기본값으로 삼아 실제 검색에 쓸 중국어 검색어를 정확히 하나 생성합니다. 기본값은 직접 수정할 수 있습니다. 결과가 없으면 Google Lens·Google·네이버쇼핑에서 동일 상품을 탐색할 수 있지만 외부 미디어를 자동 다운로드하지는 않습니다.
4. **미디어** — 선택한 게시물의 원본 소스 스트림을 MP4로 저장하고 대표 장면을 JPG로 추출합니다. 영상 1개와 대표 이미지 1장을 선택하거나, 적합한 자료가 없으면 미디어 없이 진행합니다.
5. **문구** — 미디어가 있으면 화면에 실제로 보이는 장면과 상품 정보를 함께 사용하고, 없으면 상품 정보만 사용합니다. 결정된 근거로 고정 6개 페르소나와 선택형 커스텀 페르소나의 3~5문장, 150~250자 한국어 본문을 한 번 생성하며, 선택한 버전만 다시 만들 수도 있습니다. 고정된 조회수 패턴은 사용하지 않고, 사용자가 별도로 제공한 참고 Threads의 구조와 말투만 요청할 때마다 참고 목록에 추가합니다.
