...
→ STEP 03 중국어 검색어 1개로 RedNote 영상 검색
→ STEP 04 MP4·대표 JPG 선택 또는 미디어 없이 진행
→ STEP 05 결정된 근거로 6개 페르소나+커스텀 문구 생성·선택
...
2. 쿠팡 파트너스 검색 결과를 먼저 최대 10개 비교합니다. 선택지를 더 보려면 `Chrome에서 더 보기`를 눌러 로그인된 Chrome 검색 결과를 합친 최대 30개 목록을 확인합니다.
...
## 4. STEP 03 — RedNote 중국어 영상 검색

쿠팡 상세 분석에서 확인된 상품 타이틀을 기본값으로 삼아 RedNote에서 실제로 사용할 중국어 검색어를 정확히 하나 만듭니다. 검색창의 기본값은 필요하면 직접 수정할 수 있습니다. `검색어 다시 만들기`를 누르면 검색어가 추가되는 것이 아니라 기존 검색어를 새 검색어 하나로 교체합니다.
...
동일 상품 미디어가 없으면 `다른 곳에서 같은 상품 찾기`에서 Google Lens, Google, 네이버쇼핑을 열 수 있습니다. 이 기능은 후보를 찾는 탐색 전용이며 외부 영상이나 이미지를 자동으로 내려받지 않습니다. 공식 브랜드·제조사 자료나 직접 소유한 자료처럼 재사용 권한이 확인된 미디어만 사용하세요. 적합한 자료가 없으면 `미디어 없이 문구 만들기`를 눌러 상품 정보만으로 문구를 생성할 수 있습니다.

`Chrome에서 확인`, 쿠팡 검색 `Chrome에서 더 보기`, RedNote 검색은 macOS 로컬 실행 전용 기능입니다. Chrome 메뉴의 `보기 → 개발자 → Apple Events의 자바스크립트 허용`을 켜고, `시스템 설정 → 개인정보 보호 및 보안 → 자동화`에서 실행에 사용하는 터미널 앱(필요한 경우 Python 또는 Node)이 Google Chrome을 제어하도록 허용하세요. Apple Events 권한을 거부했다면 이 설정에서 다시 켠 뒤 스튜디오를 재시작합니다.

## 5. STEP 04 — MP4 다운로드와 대표 JPG 구성
...
- 영상 1개와 대표 JPG 1장을 선택하면 대표 장면을 Codex로 분석합니다.
- 장면 분석이 실패하면 확인되지 않은 시각 정보를 문구 근거로 사용하지 않습니다.
- 미디어가 없으면 상품 정보만으로 문구를 만들고 본문과 광고 고지 댓글만 게시할 수 있습니다.

이 기능은 워터마크 제거 기능이 아닙니다. 원본 소스에 워터마크나 출처 표시가 포함되어 있으면 그대로 유지되며, 이를 제거하거나 우회하지 마세요.

## 6. STEP 05 — 페르소나별 한국어 문구

미디어 선택 또는 건너뛰기가 끝나면 Codex CLI를 비대화형으로 호출해 문구를 한 번 만듭니다. 미디어가 있으면 대표 장면에서 확인되는 동작·변화·배치와 상품 정보를 함께 사용하고, 없으면 상품 정보만 사용합니다. 본문은 3~5문장, 150~250자로 작성하며 불편한 상황에서 상품이 쓰이는 장면과 구체적인 변화 또는 반응으로 자연스럽게 이어집니다. 고정 페르소나는 다음 6개입니다.
...
고정된 조회수 패턴이나 공통 첫 문장 후보는 사용하지 않습니다. 사용자가 참고할 Threads 링크를 별도로 전달하면 원문을 복사하지 않고 첫 문장 유형, 정보 공개 순서, 말투와 리듬을 분석해 참고 목록에 추가합니다.
